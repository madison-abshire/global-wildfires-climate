...
def main() -> None:
    ...
    with st.container():
        ...
        with col3:
            ...
            with subcol1:
                median_temp = df_filtered['Temperature_C'].median().round(2) if not df_filtered.empty else 0
                st.metric('Median Temperature (C)', median_temp)
            with subcol2:
                median_wind_speed = df_filtered['Wind_Speed_kmh'].median().round(2) if not df_filtered.empty else 0
                st.metric('Wind Speed (km/h)', median_wind_speed)
            with subcol3:
                median_humidity = df_filtered['Humidity_Percent'].median().round(2) if not df_filtered.empty else 0
